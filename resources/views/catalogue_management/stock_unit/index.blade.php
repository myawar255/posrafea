...
@section('page_title', __('Stock Unit') )
...
@section('page_head')
    <div class="page-head">
        <h2 class="page-head-title">Stock Unit</h2>
        <nav aria-label="breadcrumb" role="navigation">
            <ol class="breadcrumb page-head-nav">
                <li class="breadcrumb-item">Catalogue Management</li>
                <li class="breadcrumb-item active">Stock Unit</li>
            </ol>
        </nav>
    </div>
@endsection
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-table">
                <div class="card-body">
                    <div class="noSwipe">
                        @can('create-stock-unit')
                        <div class="row m-0">
                            <div class="col-md-12 p-2 text-right">
                                <button class="btn btn-space btn-primary btn-sm" data-toggle="modal" data-target="#add-stock-unit" type="button">
                                    <i class="icon icon-left mdi mdi-plus"></i>Add New
                                </button>
                            </div>
                        </div>
                        @endcan
                        <table class="table table-striped table-hover be-table-responsive" id="stock-unit" style="width:100%">
                            <thead>
                                <tr>
                                    <th style="width:5%;">
...
@section("modals")
    ...
    @can('create-stock-unit')
    <!-- START OF ADD NEW STOCK UNIT MODAL -->
    <div class="modal fade colored-header colored-header-primary" id="add-stock-unit" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content be-loading">
                <form id="create-stock-unit-validation" action="{{ route('stock_unit.store') }}">
                    @csrf
                    <div class="modal-header modal-header-colored">
                        <h3 class="modal-title">Add New Stock Unit</h3>
                    ...
                    <div class="modal-footer">
                        ...
                        <button class="btn btn-primary store-stock-unit" type="button">Save</button>
                    </div>
                </form>
    ...
    <!-- END OF ADD NEW STOCK UNIT MODAL -->
    @endcan

    @can('view-stock-unit')
    <!-- START OF VIEW STOCK UNIT MODAL -->
    <div class="modal fade colored-header colored-header-primary" id="view-stock-unit" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content be-loading">
                <form>
                    <div class="modal-header modal-header-colored">
                        <h3 class="modal-title">View Stock Unit</h3>
                        <button class="close md-close" type="button" data-dismiss="modal" aria-hidden="true"><span class="mdi mdi-close"></span></button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
    ...
    <!-- END OF VIEW STOCK UNIT MODAL -->
    @endcan

    @can('update-stock-unit')
    <!-- START OF EDIT STOCK UNIT MODAL -->
    <div class="modal fade colored-header colored-header-primary" id="edit-stock-unit" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content be-loading">
                <form id="edit-stock-unit-validation">
                    <div class="modal-header modal-header-colored">
                        <h3 class="modal-title">Edit Stock Unit</h3>
                        <button class="close md-close" type="button" data-dismiss="modal" aria-hidden="true"><span class="mdi mdi-close"></span></button>
                    </div>
                    @canany(['delete-stock-unit'])
                    <div class="btn-group btn-group-justified btn-space" role="group">
                        <a class="btn btn-danger delete" href="javascript:void(0)">Delete Stock Unit</a>
                    </div>
                    @endcanany
                    <div class="modal-body">
                        <div class="row">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-secondary md-close" type="button" data-dismiss="modal">Close</button>
                        <button class="btn btn-primary update-stock-unit" type="button">Save</button>
                    </div>
                </form>
                <div class="be-spinner">
                    <svg width="40px" height="40px" viewBox="0 0 66 66" xmlns="http://-www.w3.org/2000/svg">
...
@section('page_level_scripts')
	...
	<script type="text/javascript">
		$(document).ready(function(){
            ...
            var selected_rows = [];
            //Table init
            let stock_unit = $('#stock-unit').DataTable({
                "pageLength": 10,
                "processing": true,
                "serverSide": true,
                "searchDelay": 1000,
                "ajax": {
                    "url": "{{route('stock_unit.get')}}",
                    "method": "POST",
                    'headers': {
            ...
            $(".dataTable").on("click", "th.select-checkbox", function(e) {
                ...
                if(e.target.tagName == "INPUT"){
                    if ($(this).hasClass("selected")) {
                        var data = stock_unit.rows({filter: 'applied', page: 'current'}).every( function ( idx, data, node ) {
            ...
            stock_unit.on("click", "td.select-checkbox", function(e) {
                if(e.target.tagName == "INPUT"){
            ...
            @can('view-stock-unit')
            /* VIEW STOCK UNIT CODE */
            stock_unit.on("click", ".view-stock-unit", function(e) {
                var route = $(this).attr("data-view-route");
                var modal = $("#view-stock-unit");
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.success) {
                            ...
                            $(modal).find("h3.modal-title").html("View Stock Unit (#"+data.data.id+" - "+data.data.name+")");
            ...
            @can('update-stock-unit')
            /* EDIT STOCK UNIT CODE */
            stock_unit.on("click", ".edit-stock-unit", function(e) {
                var route = $(this).attr("data-edit-route");
                var modal = $("#edit-stock-unit");
                var form = $(modal).find("form");
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.success) {
                            var data_keys = Object.keys(data.data);
                            var data_values = Object.values(data.data);
                            $(modal).find("h3.modal-title").html("Edit Stock Unit (#"+data.data.id+" - "+data.data.name+")");
                            $(modal).find("form").attr("action",data.data.update_stock_unit);
                            @can('delete-stock-unit')
            ...
            @can('delete-stock-unit')
            $("#stock-unit, #edit-stock-unit").on("click", ".delete", function(){
                var deleteBtn = this;
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if(data.success) {
                                ...
                                $.gritter.add({
                                    title: '',
                                    text: 'Stock unit successfully deleted',
            ...
            @can('update-stock-unit')
            $("#edit-stock-unit").on("click", ".update-stock-unit", function(){
                var modal = $("#edit-stock-unit");
                var form = $(modal).find("form");
            ...
            @can('create-stock-unit')
            /* ADD NEW STOCK UNIT CODE */
            $("#add-stock-unit").on("click", ".store-stock-unit", function(){
                var modal = $("#add-stock-unit");
                var form = $(modal).find("form");
                var action = $(form).attr("action");
                var form_data = new FormData(form[0]);
                $.ajax({
            ...
            $('#add-stock-unit').on('hidden.bs.modal', function(e) {
